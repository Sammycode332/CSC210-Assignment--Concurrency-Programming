...
public final class ConsoleReport {
    ...
    public static void printPapers(TopicResult r) {
        ...
        serp.mine.TopicSummary s = r.summary();
        if (!s.overview().isBlank() || !s.keyPoints().isEmpty()) {
            System.out.println();
            System.out.println(" SUMMARY OF RELEVANT CONTENT");
            if (!s.overview().isBlank()) {
                System.out.println("   " + s.overview());
            }
            for (serp.mine.TopicSummary.KeyPoint kp : s.keyPoints()) {
                System.out.printf("   [%d] %s%n", kp.paperRank(), kp.text());
            }
            System.out.println();
        }
